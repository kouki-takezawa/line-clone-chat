...
export default function Home() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (trimmed === todaysUnlockPhrase()) {
      // Session cookie (no Max-Age): cleared once the browser/PWA process is
      // fully closed, so the disguise reappears next time the app is
      // reopened instead of staying unlocked forever like localStorage
      // would. proxy.ts checks for this on every request.
      document.cookie = "disguise_unlocked=1; path=/; samesite=lax";
      router.push("/login");
      return;
    }
